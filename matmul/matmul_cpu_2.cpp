...
template <int rows, int columns, int inners>
inline void matmulImplLoopReorder(const float *left, const float *right,
                                float *result) {
  for (int row = 0; row < rows; row++) {
    for (int inner = 0; inner < inners; inner++) {
      for (int col = 0; col < columns; col++) {
...
int main(){
    ...
    matmulImplLoopReorder<dim, dim, dim>(left, right, result);
    end = chrono::high_resolution_clock::now();

    duration = chrono::duration_cast<chrono::milliseconds>(end - start);
    cout << "Time taken for matrix multiplication with Loop Reorder: " << (double)duration.count() << " millisecs" << endl;
    cout << "------------------------" << endl;
